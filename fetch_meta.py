"""
Meta Ads API + MyASP CV (Google Sheet) — FrancDirect専用
キャンペーンID でフィルタリングして他クライアントのデータを除外
"""
...
def meta(params):
    r = requests.get(BASE, params={**params, "access_token": ACCESS_TOKEN})
    body = r.json()
    if "error" in body:
        raise RuntimeError(f"Meta API: {body['error']['message']}")
    return body.get("data", [])

def cv_from_actions(actions, atype="lead"):
    for a in (actions or []):
...
# ── Meta: 各期間サマリー（FrancDirectのみ） ───────────────────────────
today_str   = date.today().strftime("%Y-%m-%d")
month_start = date.today().strftime("%Y-%m-01")
day30_start = (date.today().replace(day=1).__class__.fromordinal(
    date.today().toordinal() - 29)).strftime("%Y-%m-%d")

def fetch_summary(preset):
    rows = meta({
        "fields":    FIELDS,
        "date_preset": preset,
        "level":     "campaign",        # キャンペーン単位で取得
        "filtering": CAMP_FILTER,       # FrancDirectのみ
    })
    # 複数行返る場合は合算（通常は1行）
    if not rows:
    ...
    if merged["reach"] > 0:
        merged["ctr"] = round(merged["clicks"] / merged["impressions"] * 100, 2) if total_imp > 0 else 0
    if merged["clicks"] > 0:
...
print(f"📡 FrancDirect (キャンペーンID: {CAMPAIGN_ID}) のデータを取得中...")
...
print("✅ 完了（FrancDirectのみ集計）")
print(f"   今日    : ¥{s_today['spend']:,}  MyASP CV:{s_today['cv_myasp']}")
print(f"   今月    : ¥{s_month['spend']:,}  MyASP CV:{s_month['cv_myasp']}")
print(f"   トータル: ¥{s_total['spend']:,}  MyASP CV:{s_total['cv_myasp']}")
